...
class AnsweringMachine(Generic[_T], metaclass=ReferenceAM):
    ...
    def parse_all_options(self, mode: int, kargs: Any) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    def send_reply(self, reply: _T, send_function: Optional[Callable[..., None]] = None) -> None:
        if send_function:
            send_function(reply)
        else:
    ...
    def reply(self, pkt: Packet, send_function: Optional[Callable[..., None]] = None, address: Optional[Any] = None) -> None:
...
class AnsweringMachineTCP(AnsweringMachine[Packet]):
    ...
    def make_reply(self, req: Packet, address: Optional[Any] = None) -> Packet:
        return req
